...
final static String TITLE = "D\u00e9mineur";
...
public void draw()
{
  if (partie == 1){ //Le joueur est dans une partie
    ...
    int musique = PApplet.parseInt(random(3));
...
public void preparerMines(){
  ...
  {
    ligne = PApplet.parseInt(random(largeur));
    colonne = PApplet.parseInt(random(hauteur));
...
public void debut(){
  ...
  int fond = PApplet.parseInt(random(3));
...
public void menu(){
  ...
  textSize(25);
  fill(0xffFFFFFF);
  text("Volume",1055,420);
  textSize(20);
  fill(0xffff8000);
  text("Cr\u00e9\u00e9 par : ",15,670);
  text("L\u00e9o-Paul Huar & Sylvain Buche",15,690);
  text("Projet cr\u00e9\u00e9 avec Processing 3.2.1",15,710);
  textSize(15);
  fill(0xffFFFFFF);
  text("Version 1.0.0",15,730);
}

public void boutonQuitter(int x, int y){ //Affiche le bouton Quitter
  image(quitterIm,x,y);
}

public void resultat(boolean gagner){
  if (gagner == true){
...
public void caseBlanche(int x, int y)
{
...
public void volume(boolean monter){
...
public void changeAppTitle(String title) {
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "--present", "--window-color=#666666", "--hide-stop", "Demineur" };
    if (passedArgs != null) {
